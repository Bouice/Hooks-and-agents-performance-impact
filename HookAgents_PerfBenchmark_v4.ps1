...
foreach ($AppName in $AppsToBench.Keys) {
    ...
    $Results += [PSCustomObject]@{ Test="Launch_$AppName"; Sec=[math]::Round($T.TotalSeconds, 2); Base=$Baselines["Launch_$AppName"]; Info="Cold Start + 5s" }
}
